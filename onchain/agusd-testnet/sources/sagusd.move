...
///
/// A `StakingVault` custodies the staked agUSD and owns the sagUSD
/// `TreasuryCap`. sagUSD is a **share** of the vault (ERC-4626 style):
///
///     NAV per share = total_assets / total_shares
///
/// Staking and unstaking are priced at the current NAV, so the swap is **not
/// 1:1** once yield has accrued. The Allocation Engine deposits private-credit
/// yield into the vault via `accrue_yield` — this raises `total_assets` without
/// minting shares, so every sagUSD becomes redeemable for more agUSD over time.
///
///   • stake(agUSD)   → sagUSD   (shares = amount · total_shares / total_assets)
///   • unstake(sagUSD)→ agUSD    (assets = shares · total_assets / total_shares)
///   • accrue_yield(agUSD)       → NAV per share rises
module agusd_testnet::sagusd;

use agusd_testnet::agusd::AGUSD;
...
/// The staking vault. Holds staked agUSD and owns the sagUSD supply.
public struct StakingVault has key {
    id: UID,
    assets: Balance<AGUSD>,          // total agUSD backing sagUSD
    treasury: TreasuryCap<SAGUSD>,   // sagUSD shares; total_supply = total shares
}
...
/// Stake `agusd` and receive sagUSD shares priced at the current NAV.
...
/// Unstake `sagusd` and receive agUSD at the current NAV (includes accrued yield).
...
/// Allocation Engine books yield: deposit agUSD into the vault WITHOUT minting
/// shares, so the NAV per share rises for every holder.
